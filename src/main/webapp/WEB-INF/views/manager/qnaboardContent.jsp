					...
					<div class="panel">
						...
						<input type="hidden" id="qnaBoardSeq" name="qnaBoardSeq" value="${qnaBoard.qnaBoardSeq}">	
							...
							<div class="panel-body">
								...
								<br>
								<button type="button" id ="qnaDeleteBtn" name="qnaDeleteBtn"class = "btn btn-primary btn-toastr" >글 삭제</button>
							</div>
							<br>
							<br>
							<br>
							<div class="panel-body">
								<h5>답변 내용</h5>
							<c:choose>
								<c:when test="${!empty qnaBoard.qnaReplyList}">
									<c:forEach var="qnaReplyList" items="${qnaBoard.qnaReplyList}" varStatus="status">
										
										<div class="well">
											<p class="text-left">작성자 : <code>${qnaReplyList.managerId} </code> </p>
											<br>
											<p class="text-left">답변 내용 : <code>${qnaReplyList.replyContent} </code> </p>
											<br>
											<p class="text-right">작성일 : <code>${qnaReplyList.regDate} </code> </p>
...
<c:if test="${qnaBoard eq null}">
	alert("잘못된 접근입니다.");
	location.href="/manager/qnamanagement";
</c:if>
$(document).ready(function()
{
	...
	{
		if(confirm("문의글을 삭제 하시겠어요?") == true)
		{
			qnaDeleteAjax(qnaBoardSeq);	
		}
	});
});
...
function qnaDeleteAjax(qnaBoardSeq)
{
	$.ajax
	({
		type:"POST",
		...
		success:function(response)
		{
			if(!icia.common.isEmpty(response))
			{
				...
				{
					alert("삭제가 완료되었습니다.");
					location.href = "/manager/qnaqnamanagement";	
				}
				else
				{
					alert("에러가 발생했습니다. 다시 시도해주세요.");
